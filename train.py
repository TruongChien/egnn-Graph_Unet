...
class ModelTrainer(object):
    def __init__(self,
        ...
        # set optimizer
        self.optimizer = torch.optim.Adam(params=self.module_params,
                                    lr=tt.arg.lr,
    ...
    def train(self):
        ...
        for iter in range(self.global_step + 1, tt.arg.train_iteration + 1):
            ...
            if tt.arg.test_transductive:
                full_logit = self.gnn_module(node_feat=full_data, edge_feat=init_edge)
            else:
                evaluation_mask[:, num_supports:, num_supports:] = 0  # ignore query-query edges, since it is non-transductive setting

                full_logit = torch.zeros(tt.arg.meta_batch_size, 2, num_samples, num_samples).to(tt.arg.device)
                ...
                logit = self.gnn_module(node_feat=input_node_feat, edge_feat=input_edge_feat)

                logit = logit.view(tt.arg.meta_batch_size, num_queries, 2, num_supports + 1, num_supports + 1)
            ...
            query_node_pred = torch.bmm(full_logit[:, 0, num_supports:, :num_supports], self.one_hot_encode(tt.arg.num_ways_train, support_label.long())) # (num_tasks x num_quries x num_supports) * (num_tasks x num_supports x num_ways)
            query_node_accr = torch.eq(torch.max(query_node_pred, -1)[1], query_label.long()).float().mean()


            # update model
            total_loss = query_edge_loss
            ...
            # logging
            tt.log_scalar('train/edge_loss', query_edge_loss, self.global_step)
            tt.log_scalar('train/edge_accr', query_edge_accr, self.global_step)
            tt.log_scalar('train/node_accr', query_node_accr, self.global_step)
...
if __name__ == '__main__':
    ...
    tt.arg.dataset_root = '/nfs-pku/zhangjunbo/egnn+diff_pool'
