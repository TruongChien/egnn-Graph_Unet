...
class GraphNetwork(nn.Module):
    def __init__(self,
        ...
        for l in range(self.num_layers):
            # set edge to node
            edge2node_net_down = NodeUpdateNetwork(in_features=self.in_features if l == 0 else self.node_features,
                                              num_features=self.node_features,
                                              dropout=self.dropout if l < self.num_layers-1 else 0.0)

            # set node to edge
            node2edge_net_down = EdgeUpdateNetwork(in_features=self.node_features,
                                              num_features=self.edge_features,
                                              separate_dissimilarity=False,
                                              dropout=self.dropout if l < self.num_layers-1 else 0.0)

            self.add_module('edge2node_net_down{}'.format(l), edge2node_net_down)
            self.add_module('node2edge_net_down{}'.format(l), node2edge_net_down)
       
        # up layers
        for l in range(self.num_layers-1):
            # set edge to node
            edge2node_net_up = NodeUpdateNetwork(in_features=self.node_features,
                                              num_features=self.node_features,
                                              dropout=self.dropout if l < self.num_layers-2 else 0.0)

            # set node to edge
            node2edge_net_up = EdgeUpdateNetwork(in_features=self.node_features,
                                              num_features=self.edge_features,
                                              separate_dissimilarity=False,
                                              dropout=self.dropout if l < self.num_layers-2 else 0.0)

            self.add_module('edge2node_net_up{}'.format(l), edge2node_net_up)
            self.add_module('node2edge_net_up{}'.format(l), node2edge_net_up)
        
        # pooling layers
        for l in range(self.num_layers-1):
            topk_pooling = TopKPooling(in_channels=self.node_features, 
                                       ratio=self.pool_ratios)
            self.add_module('pools{}'.format(l), topk_pooling)
    ...
    def forward(self, node_feat, edge_feat):
        ...
        for l in range(self.num_layers):
            # (1) edge to node
            node_feat = self._modules['edge2node_net_down{}'.format(l)](node_feat, edge_feat)

            # (2) node to edge
            edge_feat = self._modules['node2edge_net_down{}'.format(l)](node_feat, edge_feat)

            # pooling layer
            if l < self.num_layers-1:
                num_nodes = node_feat.size(1)
                num_feats = node_feat.size(2)
                
                node_feat = node_feat.contiguous().view(-1, node_feat.size(-1))
                edge_feat_pos = edge_feat[:,0,:,:].contiguous().view(-1, edge_feat[:,0,:,:].size(-1))
                edge_feat_neg = edge_feat[:,1,:,:].contiguous().view(-1, edge_feat[:,1,:,:].size(-1))
                
                batch = torch.tensor([[i]*num_nodes for i in range(batch_size)], dtype=torch.long).view(-1).to(tt.arg.device)
                edge_index_from = torch.tensor([[i]*num_nodes for i in range(batch_size*num_nodes)], dtype=torch.long).view(1,-1).to(tt.arg.device)
                edge_index_to = torch.tensor([list(range(i*num_nodes, i*num_nodes+num_nodes)) * num_nodes for i in range(batch_size)], dtype=torch.long).view(1,-1).to(tt.arg.device)
                edge_index = torch.cat([edge_index_from, edge_index_to], dim=0)
                edge_weight_pos = edge_feat_pos.view(-1)
                edge_weight_neg = edge_feat_neg.view(-1)
                
                # record
                node_feats = [node_feat]
                # edge_indices = [edge_index]
                edge_weights_pos = [edge_weight_pos]
                edge_weights_neg = [edge_weight_neg]
                perms = []
                
                # topk pooling
                node_feat, edge_index, edge_weight_pos, batch, perm = self._modules['pools{}'.format(l)](node_feat, edge_index, edge_weight_pos, batch)
                _, _, edge_weight_neg, _, _, _ = self._modules['pools{}'.format(l)](node_feat, edge_index, edge_weight_neg, batch)
                
                perms += [perm]
                
                node_feat = node_feat.view(batch_size, -1, num_feats)
                
                edge_feat_pos = edge_weight_pos.view(batch_size, node_feat.size(1), node_feat.size(1))
                edge_feat_neg = edge_weight_neg.view(batch_size, node_feat.size(1), node_feat.size(1))
                edge_feat = torch.cat([edge_feat_pos.unsqueeze(1), edge_feat_neg.unsqueeze(1)], dim=1)
        
        #up layer
        for l in range(self.num_layers-1):
            j = self.num_layers - 2 - l
            
            #unpooling layer
            res = node_feats[j]
            # edge_index = edge_indices[j]
            edge_weight_pos = edge_weights_pos[j]
            edge_weight_neg = edge_weights_neg[j]
            perm = perms[j]

            up = torch.zeros_like(res)
            up[perm] = node_feat
            node_feat = res + up
            num_feats = node_feat.size(-1)
            ...
            # (1) edge to node
            node_feat = self._modules['edge2node_net_up{}'.format(l)](node_feat, edge_feat)
    
            # (2) node to edge
            edge_feat = self._modules['node2edge_net_up{}'.format(l)](node_feat, edge_feat)
            
            if l == self.num_layers-2:
                # save edge feature
                edge_feat_logit = edge_feat
